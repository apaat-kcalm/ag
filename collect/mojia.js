...
var dcys = {
	...
	'm3u8': {
		...
		'rows': [{
			...
			'name': '酷酷云资源',
			'rema': '顶部跑马灯水印',
			'apis': 'http://caiji.kuyun98.com/inc/s_ldg_kkm3u8.php',
			'tips': '<span class="layui-badge layui-bg-green">国外节点</span>',
	...
	'play': {
		...
		'rows': [{
			...
			'name': '酷酷云资源',
			'rema': '顶部跑马灯水印',
			'apis': 'http://caiji.kuyun98.com/inc/s_ldg_kkyun.php',
			'tips': '<span class="layui-badge layui-bg-green">国外节点</span>',
	...
	'fuck': {
		'head': '叉站资源专区',
		'tips': '叉站资源已关闭,如需采集请购买魔加影视模板,联系QQ<a target="_blank" href="http://wpa.qq.com/msgrd?v=3&uin=1278242793&site=qq&menu=yes">1278242793</a>',
		'rows': [{
			'flag': 'dadiapi',
			'name': '大地云资源',
			'rema': '综合,https资源',
...
var comm = {
	'lister': {
		'each': function(home, urls) {
			...
			$.each(dcys, function(list, name) {
				...
				var head = list == 'pics' ? '' : '<thead><tr><td colspan="7"><span style="float:left">' + name.head +
					'</span><span style="float:right">' + name.tips + '</span></td></tr></thead>';
				...
				$.each(name.rows, function(nums, info) {
					...
					var url1 = urls.replace('{flag}', info.flag).replace('{apis}', info.apis).replace('{ac}', 'list').replace(
						'{h}', '');
					var url2 = urls.replace('{flag}', info.flag).replace('{apis}', info.apis).replace('{ac}', 'cj').replace(
						'{h}', '24');
					var url3 = urls.replace('{flag}', info.flag).replace('{apis}', info.apis).replace('{ac}', 'cj').replace(
						'{h}', '168');
					var url4 = urls.replace('{flag}', info.flag).replace('{apis}', info.apis).replace('{ac}', 'cj').replace(
						'{h}', '');
					html += '<tr class="' + fuck + '">';
					html += list == 'pics' ? '' : '<td width="20" align="center">' + nums +
						'</td><td width="70" align="center">' + info.tips + '</td>';
					if (list == 'pics') {
						html += '<td colspan="7" style="padding:0"><a target="_blank" href="' + info.urls + '" title="' + info.name +
							'"><img src="' + info.pics + '"  alt="' + info.tips + '" style="max-width:100%;width:100%;" /></a></td>';
					} else if (list == 'advs') {
						html += '<td><a class="conceal" target="_blank" href="' + info.urls + '">' + info.name + '【' + info.rema +
							'】</a></td>';
						html += '<td width="60" align="center"><a target="_blank" href="' + info.url1 + '">' + info.tip1 +
							'</a></td></td>';
						html += '<td width="60" align="center"><a target="_blank" href="' + info.url2 + '">' + info.tip2 +
							'</a></td>';
						html += '<td width="60" align="center"><a target="_blank" href="' + info.url3 + '">' + info.tip3 +
							'</a></td>';
						html += '<td width="60" align="center"><a target="_blank" href="' + info.url4 + '">' + info.tip4 +
							'</a></td>';
					} else if (list == 'news' || list == 'star') {
						html += '<td><a href="' + url1 + '&mid=' + (list == 'news' ? 2 : 8) + '">' + info.name + '【' + info.rema +
							'】</a></td>';
						html += '<td width="60" align="center"><a href="javascript:;" class="timming" data-name="当日采集：' + info.name +
							'【' + name.head + '】' + '" data-flag="' + info.flag + list + '" data-param="' + url2.replace(home, '') +
							'">定时采集</a></td>';
						...
						html += '<td><a href="' + url1 + (list == 'down' ? '&param=JmN0PTE' : '') + '">' + info.name + '【' + info.rema +
							'】</a></td>';
						html += '<td width="60" align="center"><a href="javascript:;" class="player" data-name="' + info.name +
							'" data-type="' + list + '" data-flag="' + info.coll + '" data-desc="' + info.rema + '" data-tips="' +
							info.tips.match(/[\u4e00-\u9fa5]{2,}/g) + '">' + type + '配置</a></td>';
						html += '<td width="60" align="center"><a href="javascript:;" class="timming" data-name="当日采集：' + info.name +
							'【' + name.head + '】' + '" data-flag="' + info.flag + list + '" data-param="' + url2.replace(home, '') +
							'">定时采集</a></td>';
						html += '<td width="60" align="center"><a href="' + url2 + '">采集当天</a></td>';
		...
		'init': function() {
			$(document).on('click', '.timming', function() {
				...
				layer.confirm('确认添加到定时任务吗？', {
					...
					$.post(vfed.tpls + 'asset/fed/create.php?id=tim&name=' + vfed.name, 'name=' + that.attr('data-name') +
						'&flag=' + that.attr('data-flag') + '&param=' + encodeURIComponent(that.attr('data-param')),
						function(data) {
		...
		'init': function() {
			$(document).on('click', '.player', function() {
				...
				layer.confirm('确认添加到' + type + '器中吗？', {
					...
					$.post(vfed.tpls + 'asset/fed/create.php?id=vod&name=' + vfed.name, 'name=' + that.attr('data-name') +
						'&type=' + that.attr('data-type') + '&flag=' + encodeURIComponent(that.attr('data-flag')) + '&tips=' +
						m3u8 + '&desc=' + that.attr('data-desc'),
						function(data) {
							if (data.msg) layer.alert(data.msg + '！如未生效请手动清理缓存');
							else layer.alert('请求失败');
						}).error(function(data) {
	...
	'search': {
		'init': function(urls) {
			...
			$(document).on('click', '.server', function() {
				...
				layer.confirm('更新前请备份' + $(this).attr('data-href'), {
					...
					$.post(vfed.tpls + 'asset/fed/create.php?id=cop&name=' + vfed.name, 'urls=&news=' + encodeURIComponent(that.attr(
						'data-copy')) + '&nows=' + encodeURIComponent('../../../../' + that.attr('data-href')), function(data) {
						if (data.msg) layer.alert(data.msg + '！如未生效请手动清理缓存');
						else layer.alert('请求失败');
					}).error(function(data) {
						if (data.status == 404) {
		...
		'post': function(urls, collec, hander) {
			...
			var xhr = $.post(vfed.tpls + 'asset/fed/create.php?id=sea&name=' + vfed.name, 'name=' + encodeURIComponent($(
				'.layui-input-search').val()) + '&apis=' + encodeURIComponent(collec[hander].apis), function(data, status) {
				$('.layui-collect tfoot tr td').html('【' + collec[hander].name + '】搜索到' + data.length +
					'条相关资源&nbsp;&nbsp;&nbsp;<span></span>');
				if (data.length > 0) {
					var html = '';
					var url1 = urls.replace('{flag}', collec[hander].flag).replace('{apis}', collec[hander].apis).replace('{ac}',
						'list').replace('{h}', '');
					for (var i = 0; i < data.length; i++) {
						html += '<tr>';
						html += '<td width="30" align="center">' + (hander + 1) + '</td>';
						html += '<td width="90" align="center"><span class="layui-badge layui-bg-green">' + collec[hander].name +
							'</span></td>';
						html += '<td><a href="javascript:;" class="dataurl" data-urls="' + url1 + '&wd=' + $('.layui-input-search').val() +
							'" data-head="' + collec[hander].name + '【' + collec[hander].rema + '】">' + data[i].name + '【' + data[i].note +
							'】</a></td>';
						html += '<td width="60" align="center"><a href="javascript:;" class="player" data-name="' + collec[hander].name +
							'" data-type="' + $('select[name="collect"] option:selected').val() + '" data-flag="' + collec[hander].coll +
							'" data-desc="' + collec[hander].rema + '" data-tips="' + collec[hander].tips.match(/[\u4e00-\u9fa5]{2,}/g) +
							'">播放配置</a></td>';
